...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    try:
        ind_nz = torch.nonzero(prediction[:,:,4]).transpose(0,1).contiguous()  # nonzero返回input非零元素的索引 contiguous使内存连续
    except:
        return 0
    
    # convert coordinate to absolute
    box_a = prediction.new(prediction.shape)
    box_a[:,:,0] = (prediction[:,:,0] - prediction[:,:,2]/2) # x = x - w/2
    box_a[:,:,1] = (prediction[:,:,1] - prediction[:,:,3]/2) # y = y - h/2
    box_a[:,:,2] = (prediction[:,:,0] + prediction[:,:,2]/2) # w =
    ...
    for ind in range(batch_size):
        ...
        image_pred_ = image_pred[non_zero_ind.squeeze(),:].view(-1,7) # (x,y,w,h,c,max_conf,max_conf_score)
...
def write_results_half(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        #Get rid of num_classes softmax scores 
        #Add the class index and the class score of class having maximum score
        max_conf, max_conf_score = torch.max(image_pred[:,5:5+ num_classes], 1) #返回每一列最大值及索引，_score是索引也就是类别
        max_conf = max_conf.half().unsqueeze(1)
        max_conf_score = max_conf_score.half().unsqueeze(1)
        seq = (image_pred[:,:5], max_conf, max_conf_score) #([[x,y,w,h]],m_c,m_c_s)
        image_pred = torch.cat(seq, 1) #[[x,y,w,h,m_c,m_c_s]]
        ...
        #Get the various classes detected in the image
        img_classes = unique(image_pred_[:,-1].long()).half()  # 每一个anchor一个类别的列表
        

        #WE will do NMS classwise
        for cls in img_classes:
            #get the detections with one particular class
            cls_mask = image_pred_*(image_pred_[:,-1] == cls).half().unsqueeze(1) #特定类
            class_mask_ind = torch.nonzero(cls_mask[:,-2]).squeeze()  #得分不为0
            ...
            image_pred_class = image_pred_[class_mask_ind]
             ...
             #sort the detections such that the entry with the maximum objectness
             #confidence is at the top 物体置信度最大的在最前面
            conf_sort_index = torch.sort(image_pred_class[:,4], descending = True )[1]
            image_pred_class = image_pred_class[conf_sort_index]
            idx = image_pred_class.size(0)
